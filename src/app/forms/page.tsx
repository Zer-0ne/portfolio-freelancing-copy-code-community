...
const page = () => {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            await createNew(data as Data, 'form', setIsDisabled);
                                    ...
                                    <div>
                                        {
                                            ["text", 'email'].includes(field.type) ?
                                                <input onChange={handleChange} name={field.name} value={data?.[field.name] || ''} required={field.required} placeholder={field.placeholder} style={styles.customInput()} type={field.type} key={`${field.name}-${index}`} className='w-[100%]' /> : ['select'].includes(field.type) ? <DropDown
